...
const BlogPostsRightSidebar = () => {
  ...
  const [loading, setLoading] = useState(true); // opcjonalnie
  const [error, setError] = useState('');

  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch('/api/realizacje/kategorie'); // <-- Twój endpoint
          ...
          <div className="row clearfix">
            <div className="col-md-8 order-md-1">
              <div className="row clearfix">
                ...
                {/* prosty stan ładowania/błędu (nie wpływa na layout) */}
                {loading && (
